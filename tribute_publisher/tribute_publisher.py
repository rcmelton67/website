...
def get_site_domain() -> str:
    ...
    Returns:
        - http://localhost:8000 when running locally (for local development)
        - https://meltonmemorials.com for production (default)
    
    Note: Facebook and Pinterest crawlers cannot access localhost URLs.
    Social sharing will only work when the site is deployed to a publicly
    accessible domain. Use OG tag validators (ogpreview.app, ogimage.io) to
    verify tag structure during local development.
    """
    # Check for environment variable override
    env_domain = os.environ.get("TRIBUTE_SITE_DOMAIN")
    if env_domain:
        domain = env_domain
    else:
        # Default to production domain
        domain = "https://meltonmemorials.com"
    
    # Warn if using localhost (social sharing won't work)
    if "localhost" in domain or "127.0.0.1" in domain:
        print("\n" + "="*70)
        print("WARNING: Using localhost domain for OG tags")
        print("="*70)
        print("Facebook and Pinterest cannot access localhost URLs.")
        print("Social sharing previews will NOT work during local development.")
        print("\nTo test OG tag structure:")
        print("  - View page source and verify tags are in <head>")
        print("  - Use online validators: ogpreview.app or ogimage.io")
        print("\nTo enable sharing, deploy to production domain.")
        print("="*70 + "\n")
...
# ----------------------------
# PATHS — Website Sandbox output
# ----------------------------
PROJECT_ROOT = os.path.dirname(os.path.abspath(__file__))
SITE_ROOT = os.path.join(PROJECT_ROOT, "..", "Website Sandbox")
SITE_ROOT = os.path.normpath(SITE_ROOT)
OUTPUT_DIR = os.path.join(SITE_ROOT, "memorials", "pet-tributes")
TRIBUTE_DIR = OUTPUT_DIR
IMAGE_DIR = os.path.join(TRIBUTE_DIR, "images")
DATA_JSON_PATH = os.path.join(TRIBUTE_DIR, "data.json")
TEMPLATE_PATH = os.path.join(SITE_ROOT, "templates", "tribute-template.html")

# Legacy aliases for compatibility with archive/card logic
TRIBUTES_DIR = TRIBUTE_DIR
ARCHIVE_INDEX = os.path.join(TRIBUTE_DIR, "index.html")
ARCHIVE_DATA = DATA_JSON_PATH
CARDS_PER_PAGE = 15
MAX_IMAGE_WIDTH = 1200
WEBP_QUALITY = 85

# Web base for pet tributes (used in URLs)
PET_TRIBUTES_WEB_BASE = "/memorials/pet-tributes"

# CSS path used by the generated tribute pages (adjust if your live path differs)
TRIBUTE_CSS_HREF = f"{PET_TRIBUTES_WEB_BASE}/assets/mm-tribute.css"

# Placeholder source used when no image is uploaded.
PLACEHOLDER_IMAGE_FILE = os.path.join(TRIBUTE_DIR, "assets", "blank_memorial_loving_memory.png")

# Publisher's own templates (for archive pages)
...
def load_sandbox_template(path: str) -> str:
    """Load a template from the Website Sandbox."""
    with open(path, "r", encoding="utf-8") as f:
        return f.read()
...
def verify_safe_output_path(file_path: str) -> bool:
    """
    Safety protection: Ensure all generated files are written inside
    Website Sandbox/memorials/pet-tributes/
    
    Returns True if path is safe, raises ValueError if not.
    """
    normalized_path = os.path.normpath(os.path.abspath(file_path)).lower()
    normalized_output_dir = os.path.normpath(os.path.abspath(OUTPUT_DIR)).lower()
    
    # Check if the path is within OUTPUT_DIR
    try:
        common_path = os.path.commonpath([normalized_path, normalized_output_dir])
        if common_path != normalized_output_dir:
            raise ValueError(
                f"SAFETY: Attempted to write outside allowed directory.\n"
                f"File: {normalized_path}\n"
                f"Allowed: {normalized_output_dir}"
            )
    except ValueError:
    ...
    # Additional check: ensure path contains "memorials/pet-tributes"
    if "memorials" + os.sep + "pet-tributes" not in normalized_path:
        raise Exception(
            "SAFE: Path must contain 'memorials{}pet-tributes'. File: {}".format(os.sep, file_path)
        )
...
def get_entry_folder(entry: dict) -> str:
    """Legacy: no folder structure; tributes at TRIBUTE_DIR/{slug}/"""
    return ""


def get_entry_web_base(entry: dict) -> str:
    """Web path for a tribute: /memorials/pet-tributes/{slug}/"""
    slug = (entry.get("slug") or "").strip()
    return f"{PET_TRIBUTES_WEB_BASE}/{slug}/" if slug else ""


def find_tribute_folder(slug: str, folder_hint: str = "") -> str:
    """Tribute folder: memorials/pet-tributes/{slug}/"""
    slug = (slug or "").strip()
    return os.path.join(TRIBUTE_DIR, slug)
...
def save_data(items: list[dict]):
    ...
    verify_safe_output_path(ARCHIVE_DATA)
...
def prune_entries_missing_folders(items: list[dict]) -> tuple[list[dict], list[str]]:
    ...
    Also removes orphaned folders (folders that exist but aren't in data.json).
    """
    kept = []
    removed_slugs = []
    
    # Get all valid slugs from data.json
    valid_slugs = {(item.get("slug") or "").strip() for item in items if item.get("slug")}
    
    # Check each entry in data.json
    for item in items:
    ...
    # Find and remove orphaned folders (folders that exist but aren't in data.json)
    if os.path.exists(TRIBUTE_DIR):
        for folder_name in os.listdir(TRIBUTE_DIR):
            folder_path = os.path.join(TRIBUTE_DIR, folder_name)
            # Skip known non-tribute folders
            if folder_name in ["assets", "images", "submit", "_trash", "cat", "dog", "rat"]:
                continue
            # Skip pagination folders
            if folder_name.startswith("page-"):
                continue
            # Skip files (not folders)
            if not os.path.isdir(folder_path):
                continue
            # If folder exists but slug is not in data.json, it's orphaned
            if folder_name not in valid_slugs:
                try:
                    shutil.rmtree(folder_path, ignore_errors=True)
                    print(f"Removed orphaned folder: {folder_name}")
                except Exception as e:
                    print(f"Warning: Could not remove orphaned folder {folder_name}: {e}")
...
def page_url(page_num: int) -> str:
    return page_url_for_prefix(page_num, f"{PET_TRIBUTES_WEB_BASE}/")
...
def css_href_for_page(page_num: int) -> str:
    ...
    return f"{PET_TRIBUTES_WEB_BASE}/assets/mm-tribute.css"


def build_pagination(current: int, total: int) -> str:
    return build_pagination_for_prefix(current, total, f"{PET_TRIBUTES_WEB_BASE}/")
...
def build_archive_schema(base_url: str, tributes: list[dict]) -> str:
    ...
    schema = {
        ...
        "url": f"{base_url}{PET_TRIBUTES_WEB_BASE}/",
        "mainEntity": {
            "@type": "ItemList",
...
def build_archive_full_html(cards_html: str, current_page: int, total_pages: int, tribute_entries: list[dict]) -> str:
    ...
    og_image = f"{SITE_DOMAIN}{PET_TRIBUTES_WEB_BASE}/assets/blank_memorial_loving_memory.png"
    og_url = canonical
...
""".strip()
  ...
  <link rel="icon" type="image/x-icon" href="{PET_TRIBUTES_WEB_BASE}/assets/favicon.ico">
  <link rel="shortcut icon" href="{PET_TRIBUTES_WEB_BASE}/assets/favicon.ico">
...
""".strip()
    ...
    # Load Website Sandbox header + footer (tribute-specific templates with broken links commented out)
    header_path = os.path.join(SITE_ROOT, "templates", "components", "tribute_header.html")
    footer_path = os.path.join(SITE_ROOT, "templates", "components", "tribute_footer.html")
    header_html = load_sandbox_template(header_path).replace("{{HEADER_CLASSES}}", "site-header").replace("{{PREFIX}}", "")
    footer_html = load_sandbox_template(footer_path).replace("{{PREFIX}}", "")
...
""".strip()
    ...
    archive_index_path = os.path.join(output_folder, "index.html")
    verify_safe_output_path(archive_index_path)
...
def rebuild_archive_pages(entries):
    ...
    for page_num in range(1, total_pages + 1):
        ...
        canonical = SITE_DOMAIN + page_url_for_prefix(page_num, f"{PET_TRIBUTES_WEB_BASE}/")
        pagination_prefix = f"{PET_TRIBUTES_WEB_BASE}/"
...
def rebuild_pet_type_archives(entries):
    ...
    for pet_type_slug, type_entries in grouped.items():
        ...
        for page_num in range(1, total_pages + 1):
            ...
            pagination_prefix = f"{PET_TRIBUTES_WEB_BASE}/{pet_type_slug}/"
...
def generate_sitemap(data: list[dict]):
  ...
  <url>
    ...
    for page_num in range(1, main_total_pages + 1):
        add_url(SITE_DOMAIN + page_url_for_prefix(page_num, f"{PET_TRIBUTES_WEB_BASE}/"))
    ...
    for pet_type_slug, type_entries in grouped.items():
        ...
        if type_total_pages == 0:
            type_total_pages = 1
        prefix = f"{PET_TRIBUTES_WEB_BASE}/{pet_type_slug}/"
...
"""
    ...
    verify_safe_output_path(sitemap_path)
...
def get_image_dimensions(image_path: str) -> tuple[int, int]:
    ...
    Returns (width, height) or (1200, 630) as fallback.
    """
    try:
...
def build_tribute_html(
    ...
    if input_filename:
        ...
        if og_image_width is None or og_image_height is None:
            ...
            if og_image_height is None:
                og_image_height = img_height
    else:
        image_filename = "blank_memorial_loving_memory.png"
        image_path = f"{assets_web_path}{image_filename}"
        # Use the passed-in og_image_abs directly to ensure consistency
        og_image = og_image_abs
        # Get placeholder image dimensions if not provided
        if og_image_width is None or og_image_height is None:
            placeholder_path = os.path.join(TRIBUTE_DIR, "assets", image_filename)
            img_width, img_height = get_image_dimensions(placeholder_path)
            if og_image_width is None:
                og_image_width = img_width
            if og_image_height is None:
                og_image_height = img_height
    ...
    # Clean description for OG tags - remove HTML, decode entities, normalize whitespace
    og_description_clean = re.sub(r'<[^>]+>', '', structured_meta)  # Remove HTML tags
    og_description_clean = html.unescape(og_description_clean)  # Decode HTML entities
    og_description_clean = re.sub(r'\s+', ' ', og_description_clean).strip()  # Normalize whitespace
    # Facebook recommends max 200 chars for og:description
    og_description_clean = og_description_clean[:200] if len(og_description_clean) > 200 else og_description_clean
    
    share_description = og_description_clean if og_description_clean else f"Memorial tribute for {pet_name}"
    share_subject = f"{pet_name} Memorial Tribute"
    
    # Pinterest description should be shorter (max 500 chars) and more concise
    pinterest_description = og_description_clean[:500] if len(og_description_clean) > 500 else og_description_clean
    # Create a more Pinterest-friendly description if too short
    if not pinterest_description or len(pinterest_description) < 20:
        pinterest_description = f"Memorial tribute for {pet_name}, a beloved companion remembered with love."
    
    # Pinterest share URL (first in order) - ensure image URL is absolute and accessible
    # Pinterest requires absolute URLs for both url and media parameters
    share_pinterest_url = (
        "https://pinterest.com/pin/create/button/"
        f"?url={escape_url(page_url)}"
        f"&media={escape_url(og_image)}"
        f"&description={escape_url(pinterest_description)}"
    )
    
    # Facebook share URL with quote for description
    share_facebook_url = (
        f"https://www.facebook.com/sharer/sharer.php"
        f"?u={escape_url(page_url)}"
        f"&quote={escape_url(share_description)}"
    )
    
    # Email share body - formatted with clickable links
    # Most email clients will auto-link URLs if they're on their own lines
    # Format: Put URLs on separate lines so email clients can auto-detect and link them
    email_body_lines = [
        share_description,
        "",
        "View the tribute:",
        page_url,  # URL on its own line for auto-linking
        ""
    ]
    if og_image:
        email_body_lines.append("View the image:")
        email_body_lines.append(og_image)  # URL on its own line for auto-linking
        email_body_lines.append("")
    email_body_lines.append("---")
    email_body_lines.append("Shared from Melton Memorials")
    ...
    share_email_url = (
        ...
        f"&body={escape_url(share_email_body)}"
    )
    ...
    # Ensure dimensions are set (fallback if not detected)
    if og_image_width is None:
        og_image_width = 1200
    if og_image_height is None:
        og_image_height = 630
    ...
    breadcrumb_data = {
        ...
        "itemListElement": [
            ...
            },
            {
                ...
                "position": 2,
                "name": "Pet Tributes",
                "item": f"https://meltonmemorials.com{PET_TRIBUTES_WEB_BASE}/",
            },
            {
                "@type": "ListItem",
                "position": 3,
    ...
    # Wrap with Website Sandbox site-wide header/footer.
    # Facebook and Pinterest require OG tags in specific order and format
    # Ensure og:image is absolute HTTPS URL and comes before og:image:secure_url
    head_meta = f"""  <title>{escape_html(title)}</title>
  ...
  <meta property="og:description" content="{escape_html(og_description_clean)}">
  ...
  <meta name="twitter:description" content="{escape_html(og_description_clean)}">
...
{breadcrumb_json}
  ...
  <script src="/memorials/pet-tributes/assets/mm-tribute.js" defer></script>"""

    # Load Website Sandbox header and footer (tribute-specific templates with broken links commented out)
    header_path = os.path.join(SITE_ROOT, "templates", "components", "tribute_header.html")
    footer_path = os.path.join(SITE_ROOT, "templates", "components", "tribute_footer.html")
    header_html = load_sandbox_template(header_path).replace("{{HEADER_CLASSES}}", "site-header").replace("{{PREFIX}}", "")
    footer_html = load_sandbox_template(footer_path).replace("{{PREFIX}}", "")
...
def rebuild_single_tribute_page(entry: dict, tribute_message_override: str = ""):
    ...
    # Priority: override > full_message from data.json > extract from HTML > excerpt fallback
    ...
    elif entry.get("full_message"):
        # Use full message from data.json if available
        tribute_message_html = parse_safe_markdown(entry.get("full_message", ""))
...
def relative_filename_from_url(url: str) -> str:
    ...
    if url.startswith(SITE_DOMAIN + PET_TRIBUTES_WEB_BASE + "/"):
        # last segment
        return url.split("/")[-1]
    return url
...
class TributePublisherApp:
    ...
    def edit_selected_tribute(self):
        ...
        # Priority: full_message from data.json > extract from HTML > excerpt fallback
        full_tribute_message = ""
        if entry.get("full_message"):
            full_tribute_message = entry.get("full_message", "").strip()
        else:
            try:
                tribute_folder = find_tribute_folder(slug, entry.get("folder", ""))
                index_path = os.path.join(tribute_folder, "index.html")
    ...
    def delete_selected_tribute(self):
        ...
        for slug in slugs:
            entry = next((t for t in tributes if t.get("slug") == slug), {})
            
            # Remove tribute folder and all contents
            tribute_folder = find_tribute_folder(slug, entry.get("folder", ""))
...
""", subtype="html")
    ...
    def generate(self):
        ...
        entry = {
            ...
            "years_pretty": years_pretty,
            "excerpt": excerpt,
            "full_message": tribute_msg,  # Store full message for tribute pages
            "first_name": first_name,
            "state": state,
        ...
        messagebox.showinfo(
            ...
            f"{TRIBUTES_DIR}\n"
            f"to your server's {PET_TRIBUTES_WEB_BASE}/ directory."
        )
